...
extern EvtScript N(EVS_TakeTurn_Platform);
extern EvtScript N(EVS_TakeTurn_Ground);
extern EvtScript N(EVS_HandleEvent_Platform);
...
EvtScript N(EVS_Init) = {
    ...
    Call(SetActorVar, ACTOR_SELF, AVAR_IsPlatform, AVAL_State_Platform)
    Call(GetActorVar, ACTOR_SELF, AVAR_IsPlatform, LVar0)
    IfEq(LVar0, TRUE)
        Call(SetActorVar, ACTOR_SELF, AVAR_ToppleState, AVAL_State_Platform)
        Call(BindTakeTurn, ACTOR_SELF, Ref(N(EVS_TakeTurn_Platform)))
        Call(BindHandleEvent, ACTOR_SELF, Ref(N(EVS_HandleEvent_Platform)))
...
EvtScript N(EVS_FallFromPlatform) = {
...
EvtScript N(EVS_HandleEvent_Platform) = {
    ...
    Switch(LVar0)
        ...
        CaseEq(EVENT_HIT)
            ...
            SetConst(LVar1, ANIM_BuzzyBeetle_Anim13)
            ExecWait(EVS_Enemy_Hit)
            ExecWait(N(EVS_FallFromPlatform))
...
EvtScript N(EVS_TakeTurn_Platform) = {
    Call(UseIdleAnimation, ACTOR_SELF, FALSE)
    Call(EnableIdleScript, ACTOR_SELF, IDLE_SCRIPT_DISABLE)
    Call(SetPartEventBits, ACTOR_SELF, PRT_MAIN, ACTOR_EVENT_FLAG_FLIPABLE, TRUE)
